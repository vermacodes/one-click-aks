...
# if ARM_USER_PRINCIPAL_NAME contains fdpo.onmicrosoft.com then curt different part
if [[ "${ARM_USER_PRINCIPAL_NAME}" == *"fdpo.onmicrosoft.com"* ]]; then
  CONTAINERAPP_NAME="${USER_ALIAS}-containerapp-actlabs-fdpo"
else
  CONTAINERAPP_NAME="${USER_ALIAS}-containerapp-actlabs"
fi
...
# This script deploys Azure Containerapp and add Application Settings.
gap
log "🚀 Deploying Containerapp"
...
if deploy_containerapp; then
  log "Successfully deployed Containerapp"
else
  err "Failed to deploy Containerapp"
  exit 1
fi
